        ...
        if winner != 0:
            ...
            if tournament_id and is_user_in_tournament(tournament_id, winner):
                ...
                if int(get_number_of_winners(tournament_id, cur_round)) == nopir and nopir != 1:
                    ...
                    await update_tournament_winner_if_round_finished(tournament_id, winner)
                ...
                if not is_any_user_excluded(game_id):
                    ...
                    conn.commit()
                    conn.close()
                    await bot.send_message(chat_id=winner, text=f"You got {coin} Unity Coins for winning in the game 🎁")
            delete_game(game_id)
...
async def send_random_cards_to_players(game_id):
    ...
    for player_id in players:
        ...
        # await bot.send_message(chat_id=1155076760, text=f"user id: {player_id}\nis turn:{is_user_turn(player_id, game_id)}\ncur turn: {get_current_turn_user_id(game_id)}\ngame id: {game_id}")
        if is_turn:
